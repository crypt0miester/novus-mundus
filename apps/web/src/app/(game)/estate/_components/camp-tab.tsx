...
const OPERATIVE_UNITS = [
  { label: "Laborer", multiplier: 1, field: "operativeUnit1" as const, unitType: 3 },
  { label: "Artisan", multiplier: 4, field: "operativeUnit2" as const, unitType: 4 },
  { label: "Engineer", multiplier: 16, field: "operativeUnit3" as const, unitType: 5 },
];
...
export function CampTab() {
  ...
  const handleClaimAndHire = async (reportPhase: (p: TxPhase) => void) => {
    ...
    return transact.mutateAsync({
      ...
      invalidateKeys: [["player"]],
      successMessage: `Claimed NOVI & hired ${OPERATIVE_UNITS[hireType]?.label}!`,
      onPhase: reportPhase,
    }).then((r) => r.signature);
  };
