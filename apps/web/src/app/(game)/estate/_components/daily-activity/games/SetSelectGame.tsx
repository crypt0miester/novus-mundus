 ...
 */
export function SetSelectGame({
  presentation,
  submitting,
  onSubmit,
}: SetSelectGameProps) {
  const { instruction, aLabel, bLabel, items } = presentation;
  const [selected, setSelectedAt] = useIndexedSelection<boolean>(() =>
    items.map(() => false),
  );
          ...
          return (
            <button
              ...
              className={`card flex items-center justify-between gap-2 text-left transition-colors ${
                on ? "border-amber-600 bg-amber-900/20" : "hover:border-amber-800/50"
              }`}
            >
              <div>
              ...
              <span
                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border text-xs ${
                  on
                    ? "border-amber-500 bg-amber-500/30 text-text-gold"
