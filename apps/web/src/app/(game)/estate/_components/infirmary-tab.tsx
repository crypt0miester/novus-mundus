...
const WOUNDED_UNITS = [
  { type: 0, label: "Infantry", field: "woundedDef1" as const, icon: "unit-infantry" as const },
  { type: 1, label: "Cavalry", field: "woundedDef2" as const, icon: "unit-cavalry" as const },
  { type: 2, label: "Siege", field: "woundedDef3" as const, icon: "unit-siege" as const },
  { type: 3, label: "Laborer", field: "woundedOp1" as const, icon: "unit-laborer" as const },
  { type: 4, label: "Artisan", field: "woundedOp2" as const, icon: "unit-artisan" as const },
  { type: 5, label: "Engineer", field: "woundedOp3" as const, icon: "unit-engineer" as const },
];
...
export function InfirmaryTab() {
  ...
  const selectedWounded = woundedCounts[recoverUnitType];
  const selectedWoundedMax = selectedWounded?.count ?? 0;
  ...
  return (
    <FeatureLayout
        ...
        <>
            ...
            {infirmaryLevel > 0 && (
              <div>
                ...
                <span className="font-semibold text-green-600">
                  {((infirmaryLevel * 25) / 100).toFixed(1)}%
                </span>
              </div>
            )}
            <div>
              ...
              <span
                className={`font-semibold ${totalWounded > 0 ? "text-red-400" : "text-text-primary"}`}
              >
          ...
          {totalWounded === 0 ? (
            <div className="card">
              <p className="text-sm text-text-muted">
                No wounded units to recover. Your troops are in good health.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="grid gap-2 grid-cols-3 md:grid-cols-6">
                {woundedCounts.map((u) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-lg font-semibold ${u.count > 0 ? "text-red-400" : "text-text-muted"}`}
                    >
              ...
              {selectedWounded && (
                <ShowcaseBanner
                  image="/img/banners/infirmary-banner.webp"
                  icon={selectedWounded.icon}
                  title={selectedWounded.label}
                  tag="wounded"
                >
                  <p className="text-xs italic text-zinc-300">
                    Wounded troops sit out the fight until they are tended and returned to your
                    ranks.
                  </p>
                  <p className="text-xs text-zinc-400">
                    <span className="font-mono tabular-nums text-zinc-100">
                      {selectedWounded.count.toLocaleString()}
                    </span>{" "}
                    wounded — recovers at roughly half the cost of hiring anew.
                  </p>
                </ShowcaseBanner>
              )}
            </>
          ...
          {infirmaryLevel === 0 && (
            <div className="card border-border-gold/30">
              <p className="text-sm text-text-muted">
                Build or upgrade the <span className="text-text-gold">Infirmary</span> to reduce
                recovery costs. Each level gives a 0.25% discount.
              </p>
            </div>
          )}
        </>
      ...
      aside={
        totalWounded > 0 && selectedWoundedMax > 0 ? (
          /* Recovery controls */
          <div className="card">
            ...
            <div className="mt-1 text-[11px] text-text-muted">
              50% of hire cost
              {estate?.infirmaryRecoveryDailyBps
                ? ` + ${bpsToPercent(estate.infirmaryRecoveryDailyBps)} daily buff`
                : ""}
            </div>
            <div className="mt-4">
              <TxButton
