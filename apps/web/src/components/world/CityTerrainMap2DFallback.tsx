...
export const CityTerrainMap2DFallback = forwardRef<CityTerrainMapHandle, Props>(
  function CityTerrainMap2DFallback(
    ...
    const radiusGridUnits = useMemo(() => Math.max(plotHalfW, plotHalfH), [plotHalfW, plotHalfH]);
    ...
    const focusCell = useCallback(
      (gridLat: number, gridLong: number, opts?: { scale?: number; durationMs?: number }) => {
     ...
     */
    // Baking the full 2048² (4.19M biomeAt calls) on the main thread froze
    // the UI for hundreds of ms on every city switch. We now do a two-phase
    // bake matching the 3D path (setupCityScene / useSceneSync):
    //  - A SMALL synchronous preview (TERRAIN_PREVIEW_SIZE) painted into the
    //    cache canvas immediately so the city is never blank.
    //  - The full 2048² bake runs on the shared bake Worker; on resolve its
    //    RGBA buffer is written into the same canvas and a version bump
    //    re-runs the blit effect to swap the crisp texture in.
    // Rapid city switches cancel the in-flight worker job and ignore any
    // stale resolution (a cancelled flag + key guard), so the wrong terrain
    // never paints.
    const TERRAIN_CACHE_SIZE = 2048;
    const TERRAIN_PREVIEW_SIZE = 512;
    // The cache key folds every bake input into one string so a single
    // compare decides whether the current canvas is still valid. The
    // downstream blit treats cache pixel (SIZE/2, SIZE/2) as grid (0,0).
    const terrainCacheKey = useMemo(
      () =>
        [
          cityAccount.cityId,
          biomeSeed,
          radiusGridUnits,
          biomeKnobs.waterLevelDelta,
          biomeKnobs.tempBias,
          biomeKnobs.moistureBias,
          biomeKnobs.coast,
          biomeKnobs.landmassSeed,
        ].join(":"),
    ...
    // Bumped each time the cache canvas contents change (preview paint, then
    // full-res worker swap). The blit effect depends on it so the visible
    // terrain canvas repaints from the freshest cache contents.
    const [terrainVersion, setTerrainVersion] = useState(0);

    // Blit worker-convention RGBA (row 0 = oy≈-r, the bakeBiomePixels grid
    // order) into the cache canvas, which is stored in the on-screen
    // convention (row 0 = oy≈+r). drawImage with a vertical flip reconciles
    // the row order AND scales the preview up to the cache size in one pass;
    // putImageData can't because it ignores transforms.
    const blitBakedPixels = useCallback(
      (cacheCtx: CanvasRenderingContext2D, pixels: Uint8Array, srcSize: number) => {
        const src = document.createElement("canvas");
        src.width = srcSize;
        src.height = srcSize;
        const srcCtx = src.getContext("2d");
        if (!srcCtx) return;
        const img = srcCtx.createImageData(srcSize, srcSize);
        img.data.set(pixels);
        srcCtx.putImageData(img, 0, 0);
        cacheCtx.save();
        cacheCtx.imageSmoothingEnabled = false;
        cacheCtx.setTransform(1, 0, 0, -1, 0, TERRAIN_CACHE_SIZE);
        cacheCtx.drawImage(src, 0, 0, TERRAIN_CACHE_SIZE, TERRAIN_CACHE_SIZE);
        cacheCtx.restore();
      },
      [],
    );

    // Preview + worker bake. The synchronous low-res preview paints the
    // cache immediately so the first frame is never blank; the worker then
    // bakes the full-res texture off the main thread and swaps it in.
    useEffect(() => {
      if (typeof document === "undefined") return;
      let cacheCanvas = terrainCacheRef.current?.canvas ?? null;
      if (!cacheCanvas) {
        cacheCanvas = document.createElement("canvas");
        cacheCanvas.width = TERRAIN_CACHE_SIZE;
        cacheCanvas.height = TERRAIN_CACHE_SIZE;
      }
      const cacheCtx = cacheCanvas.getContext("2d");
      if (!cacheCtx) return;

      // Synchronous preview — same bakeBiomePixels math as the worker (and
      // the 3D path), so the preview and the full-res swap register
      // pixel-for-pixel with no visible jump when the texture upgrades.
      const previewPixels = bakeBiomePixels(
        biomeSeed,
        radiusGridUnits,
        biomeKnobs,
        TERRAIN_PREVIEW_SIZE,
      );
      blitBakedPixels(cacheCtx, previewPixels, TERRAIN_PREVIEW_SIZE);
      terrainCacheRef.current = { key: terrainCacheKey, canvas: cacheCanvas };
      setTerrainVersion((v) => v + 1);

      // Full-res off-thread bake. Guarded against stale resolution: if the
      // key changed (city switch) before this resolves, the ref's key no
      // longer matches and we drop the buffer.
      let cancelled = false;
      const job = getBakeWorker().bake({
        biomeSeed,
        rgu: radiusGridUnits,
        knobs: biomeKnobs,
        texSize: TERRAIN_CACHE_SIZE,
      });
      job.promise.then((pixels) => {
        if (cancelled || !pixels) return;
        const current = terrainCacheRef.current;
        if (!current || current.key !== terrainCacheKey) return;
        const ctx = current.canvas.getContext("2d");
        if (!ctx) return;
        blitBakedPixels(ctx, pixels, TERRAIN_CACHE_SIZE);
        setTerrainVersion((v) => v + 1);
      });
    ...
    }, [terrainCacheKey, biomeSeed, radiusGridUnits, biomeKnobs, blitBakedPixels]);
    ...
    useEffect(() => {
      ...
      const terrainCache = cached && cached.key === terrainCacheKey ? cached.canvas : null;
    ...
    // scene.
    const hasAnimatedDot = useMemo(() => occupied.some((c) => c.nameColorAnim != null), [occupied]);
    useEffect(() => {
    ...
    useEffect(() => {
      ...
      if (otherWalks && otherWalks.length > 0) {
        ...
        for (const w of otherWalks) {
          ...
          const baseColor = w.nameColorHex
            ? w.nameColorAnim
              ? animatedColorToRgba(animatedColorAt(w.nameColorHex, w.nameColorAnim, walkNowMs))
              : w.nameColorHex
            : null;

          ctx.save();
          ctx.strokeStyle = baseColor ? toRgbaWithAlpha(baseColor, 0.45) : "rgba(180, 83, 9, 0.4)";
          ...
          ctx.fillStyle = baseColor ? toRgbaWithAlpha(baseColor, 0.85) : "rgba(180, 83, 9, 0.85)";
      ...
      if (travel) {
        ...
        ctx.fillStyle = travelColor ? toRgbaWithAlpha(travelColor, 0.25) : "rgba(180, 83, 9, 0.25)";
        ctx.beginPath();
      ...
      const teamMateSet =
        teamMatePubkeys && teamMatePubkeys.length > 0 ? new Set(teamMatePubkeys) : null;
      ...
      for (const cell of occupied) {
        ...
        const isTeamMate = isPlayer && !isMyPlayer && teamMateSet?.has(cell.occupant) === true;
        ...
        if (isPlayer && cell.nameColorHex) {
          fill = cell.nameColorAnim
            ? animatedColorToRgba(animatedColorAt(cell.nameColorHex, cell.nameColorAnim, nowMs))
            : cell.nameColorHex;
        } else if (isMyPlayer) {
          fill = MY_PLAYER_FILL;
        } else if (isTeamMate) {
        ...
        if (renderAsTiles) {
          ...
          ctx.lineWidth =
            (isSelectedEntity ? 3 : frameEntry ? frameEntry.ring.borderWidth : 1) * dpr;
          ...
          ctx.strokeStyle = stroke;
          ctx.lineWidth =
            (isSelectedEntity ? 2.5 : frameEntry ? frameEntry.ring.borderWidth : 1.5) * dpr;
          if (isPlayer) {
            ctx.beginPath();
      ...
      if (
        ...
        const themeParchment = computed.getPropertyValue("--parchment").trim() || "#efe2c4";
        const themeInk = computed.getPropertyValue("--ink").trim() || "#2e1f10";
        const themeInkSoft = computed.getPropertyValue("--ink-soft").trim() || "#6b4a2a";
        const themeInkFaint = computed.getPropertyValue("--ink-faint").trim() || "#b89a72";
    ...
    useEffect(() => {
      ...
      const onWheel = (e: WheelEvent) => {
        ...
        const factor = baseFactor ** intensity;
