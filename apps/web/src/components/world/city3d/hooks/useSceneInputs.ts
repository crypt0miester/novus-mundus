...
function cityTerrain(city: {
  ...
  return {
    ...
    knobs: biomeKnobsFromCity(
      city as unknown as Parameters<typeof biomeKnobsFromCity>[0],
    ),
  };
}
...
export function useSceneInputs(
  ...
  const handlePointerMove = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (markerHit && p.onActiveOccupant) {
        ...
        if (
          cell.occupantType === OCCUPANT_CASTLE &&
          cell.footprintAnchor !== true
        ) {
      ...
      const distM = Math.round(
        Math.sqrt(ox * ox + oy * oy) * METERS_PER_GRID_UNIT,
      );
  ...
  const handleClick = useCallback(
    (clientX: number, clientY: number) => {
      const r = refs.current;
      if (!r) return;
      const markerHit = raycastMarkers(
        r,
        clientX,
        clientY,
        propsRef.current.occupied,
      );
      if (markerHit) {
  ...
  const handleFrameSelected = useCallback(() => {
    ...
    const mode = r.controller.getMode();
    const dTarget =
      (mode === "iso" ? INITIAL_DISTANCE_3D : INITIAL_DISTANCE_2D) / 8;
    r.viewTween?.cancel();
    r.viewTween = runViewTween(
      r.controller,
      {
