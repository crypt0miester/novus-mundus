#!/usr/bin/env bash
# Generate city-sigil PNGs via the local Bonsai-Image-4B FastAPI server,
# driven by images/sigils/sigils.json. Raw 1024px PNGs land in
# images/sigils/raw/<id>.png — run export-sigils-to-app.sh afterwards.
#
# Usage:
#   ./generate-sigils.sh                          # every sigil in the manifest
#   ./generate-sigils.sh city-02-solterrae        # one by exact id
#   FORCE=1 ./generate-sigils.sh
#   BONSAI_URL=http://10.0.0.5:8000 ./generate-sigils.sh
#
...
STYLE_SIGIL="$(jq -r '.style.sigil' "${MANIFEST}")"
...
echo "  sigils:   ${#REQUESTED[@]} requested"
...
for req in "${REQUESTED[@]}"; do
  ...
  prompt="${PREAMBLE} ${subject} ${STYLE_SIGIL} ${TAIL}"
