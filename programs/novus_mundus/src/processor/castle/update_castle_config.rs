...
pub fn process(
    ...
    let config_type = instruction_data[6];
    ...
    match config_type {
        ...
        CONFIG_TIER_MULTIPLIER => {
            ...
            castle.tier_multiplier_bps = u16::from_le_bytes([instruction_data[7], instruction_data[8]]);
        }
        CONFIG_TREASURY_LEVEL => {
            ...
            castle.treasury_level = instruction_data[7];
        }
        CONFIG_NAME => {
            // Update castle name
            if instruction_data.len() < 55 {
            ...
            castle.name.copy_from_slice(&instruction_data[7..55]);
        }
        _ => {
            return Err(ProgramError::InvalidInstructionData);
        }
