...
pub fn process(
    ...
    let asset_data = hero_asset.try_borrow_data()?;
    ...
    let nft_data = hero_asset.try_borrow_data()?;
    ...
    let (expected_template_pda, _) = find_program_address(
    ...
    let (expected_collection_pda, _) = find_program_address(
    ...
    if mint_receipt.lamports() > 0 && mint_receipt.owner() == program_id {
        // Verify receipt PDA derivation
        let (expected_receipt_pda, _) = find_program_address(
        ...
        if mint_receipt.key() == &expected_receipt_pda {
            ...
            // Use unsafe lamport manipulation since this is a program-owned account
            unsafe {
                *mint_receipt.borrow_mut_lamports_unchecked() = 0;
                *owner.borrow_mut_lamports_unchecked() = owner.lamports().checked_add(receipt_lamports).ok_or(ProgramError::ArithmeticOverflow)?;
            }
        }
    }
