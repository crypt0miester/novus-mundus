...
pub fn process(
    ...
    let mut shop_config_data_ref = shop_config_account.try_borrow_mut()?;
    ...
    let (expected_pda, bump) = FlashSaleAccount::derive_pda(game_engine_account.address(), sale_id);
    ...
    let lamports = crate::utils::rent_exempt_const(FlashSaleAccount::LEN);
    ...
    flash_sale.payer = *payer.address();
