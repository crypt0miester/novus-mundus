...
export const SWITCHBOARD_V2_PROGRAM_ID = address(
  'SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f'
);

/** Switchboard V2 Program ID (Devnet) */
export const SWITCHBOARD_V2_PROGRAM_ID_DEVNET = address(
  '2TfB33aLaneQb5TNVwyDz3jSZXS6jdW2ARw1Dgf84XCG'
);

/** Switchboard On-Demand Program ID (Mainnet) */
export const SWITCHBOARD_ON_DEMAND_PROGRAM_ID = address(
...
export async function fetchSwitchboardResult(
  ...
  const accountInfo = await fetchAccount(rpc, aggregatorAddress);
...
export async function fetchSwitchboardPrice(
  ...
  const result = await fetchSwitchboardResult(rpc, aggregatorAddress, isOnDemand);
