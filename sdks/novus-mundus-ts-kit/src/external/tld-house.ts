 ...
 * @returns 32-byte hash
 */
export function computeReverseLookupHash(pubkey: Address): Uint8Array {
  const base58String = pubkey;
...
export function parseTldHouseData(data: Uint8Array): TldHouseData | null {
  ...
  const treasuryManager = bytesToAddress(data.subarray(8, 40));
...
export async function resolveDomain(
  ...
  const info = await fetchDomainInfo(rpc, domainName);
...
export async function reverseLookup(
  ...
  return fetchMainDomain(rpc, owner, tld);
}
