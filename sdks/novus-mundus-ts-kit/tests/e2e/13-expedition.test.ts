...
describe('Expedition System', () => {
  ...
  describe('Striking Expeditions', () => {
    it('should strike expedition to find loot', async () => {
      ...
      await sendInstructions(ctx.svm, [strikeIx], [player.keypair, ctx.daoAuthority]);
    ...
    it('should record higher cumulative score when the client supplies a stronger strike', async () => {
      ...
      await sendInstructions(
        ...
        [
          createExpeditionStrikeInstruction(
            { gameEngine: ctx.gameEngine, owner: buffed.publicKey, gameAuthority: ctx.daoAuthority.address },
            { score: 100 },
          ),
        ],
      ...
      await sendInstructions(
        ...
        [
          createExpeditionStrikeInstruction(
            { gameEngine: ctx.gameEngine, owner: unbuffed.publicKey, gameAuthority: ctx.daoAuthority.address },
            { score: 30 },
          ),
        ],
    ...
    it('should claim completed expedition', async () => {
      ...
      for (let i = 0; i < 7; i++) {
        try {
          ...
          await sendInstructions(ctx.svm, [speedupIx], [player.keypair]);
        } catch {
  ...
  describe('Aborting Expeditions', () => {
    it('should abort active expedition', async () => {
      ...
      await sendInstructions(ctx.svm, [abortIx], [player.keypair]);
