...
async function createDungeonPlayer(
  ...
  ctx: TestContext
): Promise<{ player: TestPlayer; heroMint: KeyPairSigner }> {
  ...
  await sendInstructions(ctx.svm, [mintIx], [player.keypair, heroMint]);
...
describe('Dungeon System', () => {
  ...
  const resumeIx = (owner: Address, params: { templateId: number; firstRoomType?: number }) =>
  ...
  beforeAll(async () => {
    ...
    await sendInstructions(ctx.svm, [templateIx], [ctx.daoAuthority]);
  ...
  describe('Starting Runs', () => {
    ...
    it('should reject run while another active', async () => {
      ...
      await sendInstructions(ctx.svm, [mintIx2], [player.keypair, heroMint2]);
  ...
  describe('Room Progression', () => {
    it('should advance to next room', async () => {
      ...
      await sendInstructions(ctx.svm, [advanceIx], [player.keypair, ctx.daoAuthority]);
    });
    ...
    it('should generate room based on type', async () => {
      ...
      await enterDungeon(player, heroMint.address, { templateId: DUNGEON_TEMPLATE_ID, firstRoomType: 1 });
  ...
  describe('Dungeon Combat', () => {
    ...
    it('should record the hero specialization passed at run start', async () => {
      ...
      await enterDungeon(player, heroMint.address, {
        templateId: DUNGEON_TEMPLATE_ID,
    ...
    it('should escrow the hero and stage units the player will use in combat', async () => {
      ...
      const raw = await fetchDungeonRunRaw(ctx.svm, player.playerPda);
      const run = deserializeDungeonRun(raw!.data);
      expect(run.heroMint === heroMint.address).toBe(true);
  ...
  describe('Room Types', () => {
    it('should interact with treasure room', async () => {
      ...
      await sendInstructions(ctx.svm, [interactIx], [player.keypair, ctx.daoAuthority]);
    });
    ...
    it('should use multi-attack for groups', async () => {
      ...
      await sendInstructions(ctx.svm, [multiIx], [player.keypair, ctx.daoAuthority]);
    });
  ...
  describe('Fleeing', () => {
    it('should flee from dungeon', async () => {
      ...
      await sendInstructions(ctx.svm, [fleeIx], [player.keypair]);
    });
  ...
  describe('Dungeon Completion', () => {
    it('should complete dungeon after final boss', async () => {
      ...
      try {
        await sendInstructions(ctx.svm, [completeIx], [player.keypair]);
      } catch {
  ...
  describe('Dungeon Rewards', () => {
    ...
    it('should produce a stronger boss enemy on a higher-difficulty template', async () => {
      ...
      await enterDungeon(a.player, a.heroMint.address, { templateId: DUNGEON_TEMPLATE_ID });
      const b = await createDungeonPlayer(factory, ctx);
      await enterDungeon(b.player, b.heroMint.address, { templateId: TEMPLATE_HARDER });
  ...
  describe('Dungeon Themes', () => {
    ...
    async function enterAndReadTheme(templateId: number): Promise<number> {
      const { player, heroMint } = await createDungeonPlayer(factory, ctx);
      await enterDungeon(player, heroMint.address, { templateId });
      const raw = await fetchDungeonRunRaw(ctx.svm, player.playerPda);
      const run = deserializeDungeonRun(raw!.data);
  ...
  describe('Leaderboard', () => {
    ...
    it('should claim leaderboard prize after dungeon completion', async () => {
      ...
      try {
        await sendInstructions(ctx.svm, [claimIx], [player.keypair]);
      } catch {
