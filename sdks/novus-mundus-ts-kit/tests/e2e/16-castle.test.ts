...
async function createTeamForPlayer(
  ...
  memberSlotCounter.set(teamPda, 1); // founder is slot 0, next is 1
...
describe('Castle System', () => {
  ...
  describe('Garrison Management', () => {
    ...
    beforeAll(async () => {
      ...
      await sendInstructions(ctx.svm, [claimIx], [king.keypair]);
    });
    ...
    it('should leave castle garrison', async () => {
      ...
      await sendInstructions(ctx.svm, [leaveIx], [member.keypair]);
    }, 30_000);
  ...
  describe('Court Management', () => {
    ...
    it('should appoint court member', async () => {
      ...
      await sendInstructions(ctx.svm, [appointIx], [king.keypair]);
    });
    ...
    it('should allow court member to resign', async () => {
      ...
      await sendInstructions(ctx.svm, [resignIx], [courtier.keypair]);
    });
    ...
    it('should dismiss court member', async () => {
      ...
      await sendInstructions(ctx.svm, [dismissIx], [king.keypair]);
    });
    ...
    it('should reject appointment by non-king', async () => {
      ...
      await sendInstructions(ctx.svm, [claimIx], [realKing.keypair]);
  ...
  describe('Castle Upgrades', () => {
    ...
    it('should initiate castle upgrade', async () => {
      ...
      await sendInstructions(ctx.svm, [upgradeIx], [king.keypair]);
    });
    ...
    it('should cancel castle upgrade', async () => {
      ...
      await sendInstructions(ctx.svm, [cancelIx], [king.keypair]);
    });
  ...
  describe('Castle Attacks', () => {
    ...
    it('should conquer empty-garrison castle during CONTEST (status → TRANSITIONING)', async () => {
      ...
      expect(afterAttack.transitionNewKing).toBe(attacker.playerPda);
  ...
  describe('Relieve Garrison', () => {
    it('should allow king to relieve garrison member', async () => {
      ...
      await sendInstructions(ctx.svm, [relieveIx], [king.keypair]);

      // Verify garrison contribution account is closed
      garrisonAccount = await fetchAccount(ctx.svm,garrisonPda);
  ...
  describe('Castle Transition (force_remove_king path)', () => {
    it('should complete full transition: cleanups + finalize_transition VACANT', async () => {
      ...
      expect(await fetchAccount(ctx.svm,courtPda)).toBeNull();
      ...
      if (rewardExists) {
        ...
        expect(await fetchAccount(ctx.svm,rewardPda)).toBeNull();
      }
