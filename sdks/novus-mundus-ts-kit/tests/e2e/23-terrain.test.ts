...
import { sendInstruction, expectTransactionToFail } from '../utils/transactions';
...
describe('Terrain', () => {
  ...
  describe('set_terrain', () => {
    ...
    it('should reject non-DAO signer', async () => {
      ...
      await expectTransactionToFail(ctx.svm, [ix], [fake], undefined, 'set_terrain (unauthorized)');
    });
  });
  ...
  describe('append_terrain', () => {
    ...
    it('should reject append on city without terrain', async () => {
      ...
      await expectTransactionToFail(ctx.svm, [ix], [ctx.daoAuthority], undefined, 'append_terrain (no terrain)');
    });
  });
});
