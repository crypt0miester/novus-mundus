...
describe('Encounter Cleanup', () => {
  ...
  async function spawnCommon(cityId: number, encounterIndex: number) {
    ...
    await sendInstructions(ctx.svm, [spawnIx], [ctx.daoAuthority]);
  ...
  async function cleanupIx(
    ...
    return await createCleanupEncounterInstruction({
      gameEngine: ctx.gameEngine,
      cityId,
      encounterIndex,
      gridLat,
  ...
  it('rejects cleanup before the encounter despawns', async () => {
    ...
    await expectTransactionToFail(
      ctx.svm,
      [await cleanupIx(cityId, 0, gridLat, gridLong)],
      [ctx.daoAuthority],
  ...
  it('cleans up a killed encounter (grid cell already closed by combat)', async () => {
    ...
    await sendInstructions(ctx.svm, [attackIx], [player.keypair]);
    ...
    await sendInstructions(
      ...
      [
        await createCleanupEncounterInstruction({
          ...
          gridLat,
          gridLong,
          rentRecipient: ctx.daoAuthority.address, // game engine authority
        }),
      ],
      [ctx.daoAuthority]
    );
