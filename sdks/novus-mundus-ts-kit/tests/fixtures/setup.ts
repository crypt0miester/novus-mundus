...
export interface TestContext {
  svm: LiteSVM;
  config: TestConfig;
  daoAuthority: KeyPairSigner;
  treasury: KeyPairSigner;
  kingdomId: number;
  gameEngine: Address;
  heroCollection: Address;
  cities: Map<number, Address>;
  heroTemplates: Map<number, Address>;
  researchTemplates: Map<number, Address>;
  shopConfig: Address;
  initialized: boolean;
}
...
/**
 * Create a fresh keypair signer.
 *
 * NOTE: kit's `KeyPairSigner` does not expose a raw secret-key byte array
 * (the underlying `CryptoKeyPair` private key is non-extractable), so the
 * old `keys/*.json` persistence used by the web3.js harness is not
 * reproducible here. Tests do not rely on stable keys, so each run
 * generates fresh ones. Persistence is left as a follow-up.
 */
async function loadOrCreateKeypair(_name: string): Promise<KeyPairSigner> {
...
export async function airdropIfNeeded(
  ...
  if (balance === null || balance < BigInt(minBalance)) {
    svm.airdrop(pubkey, lamports(BigInt(minBalance)));
  }
}
...
export async function setupTestContext(
  ...
  const daoAuthority = await loadOrCreateKeypair('dao-authority');
  ...
  if (fullConfig.autoSetup) {
    ...
    await airdropIfNeeded(svm, daoAuthority.address, 50 * LAMPORTS_PER_SOL);
    await airdropIfNeeded(svm, treasury.address, 1 * LAMPORTS_PER_SOL);
