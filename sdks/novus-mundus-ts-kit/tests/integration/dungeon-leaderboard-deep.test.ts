...
async function createMinimalDungeonPlayer(
  ...
  ctx: TestContext,
): Promise<{ player: TestPlayer; heroMint: KeyPairSigner } | null> {
...
async function tryCompleteDungeon(
  ctx: TestContext,
  player: TestPlayer,
  heroMint: KeyPairSigner,
  templateId: number = LB_TEMPLATE_ID,
  weekNumber?: number,
): Promise<boolean> {
  try {
    await enterDungeon(ctx, player, heroMint.address, {
      templateId,
      firstRoomType: 0,
    ...
    await sendTransaction(
      ctx.svm,
      [await createClaimDungeonInstruction(claimAccounts, claimParams)],
      [player.keypair],
    );
