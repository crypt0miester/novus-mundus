...
describe('Full Game Stress (200 players)', () => {
  ...
  let teams: { teamPda: Address; teamId: number; leader: TestPlayer; members: TestPlayer[] }[] = [];
  let castleClaims: { cityId: number; castleId: number; lord: TestPlayer }[] = [];
  let createdRallies: { creator: TestPlayer; rallyId: number; rallyPda: Address; status: 'created' | 'cancelled' }[] = [];
  ...
  it(`Phase 1: onboards ${TOTAL_PLAYERS} players across 3 tiers`, async () => {
    ...
    for (let i = 0; i < TIER_A_COUNT; i++) {
      ...
      const p = await factory.createPlayer({
        ...
        createEstate: true,
        buildings,
        customKeypair: await generateKeyPairSigner(),
      });
  ...
  it(`Phase 3: captures ${NUM_CASTLES} vacant castles + appoints courts`, async () => {
    ...
    for (const c of castleClaims) {
      const teamRecord = teams.find((t) => t.leader.publicKey === c.lord.publicKey);
        ...
        await sendTransaction(
          ...
          [
            await createUpdateLoadoutInstruction(
              ...
              {
                arenaHero: DEFAULT_ADDRESS,
                defensiveUnits: [200n, 100n, 0n],
                meleeWeapons: 0n,
                rangedWeapons: 0n,
                siegeWeapons: 0n,
                armorPieces: 0n,
    ...
    for (const challenger of arenaParticipants) {
      for (let i = 0; i < 3; i++) {
        ...
        do {
          ...
          tries += 1;
        } while (opponent.publicKey === challenger.publicKey && tries < 5);
        if (opponent.publicKey === challenger.publicKey) continue;
              ...
              await createChallengePlayerInstruction(
                {
                  ...
                  challengerHero: DEFAULT_ADDRESS,
                  challengerEstate: DEFAULT_ADDRESS,
                  defenderHero: DEFAULT_ADDRESS,
                  defenderEstate: DEFAULT_ADDRESS,
                },
  ...
  it(`Phase 6: runs ${PVP_ATTACK_ROUNDS} PvP rounds with random pairings`, async () => {
    ...
    for (let i = 0; i < PVP_ATTACK_ROUNDS; i++) {
      ...
      let attempts = 0;
      while (defender.publicKey === attacker.publicKey && attempts < 5) {
        defender = pickOne(tierA);
        attempts += 1;
      }
      if (defender.publicKey === attacker.publicKey) continue;
      ...
      const tagEvent =
        i % 2 === 0 && eventJoiners.some((p) => p.publicKey === attacker.publicKey);
  ...
  it(`Phase 7: creates ${RALLY_COUNT} rallies + lifecycle`, async () => {
    ...
    for (let i = 0; i < RALLY_COUNT; i++) {
      ...
      ctx.svm.airdrop(creator.publicKey, lamports(BigInt(20 * 1_000_000_000)));
      ...
      if ((creatorAcc.defensiveUnit1 < 50n)) {
        skippedUnits += 1;
        continue;
      }
      if (creatorAcc.arrivalTime && (creatorAcc.arrivalTime > 0n)) {
  ...
  it('Phase 9: end-state invariants hold', async () => {
    ...
    for (const p of allPlayers) {
      ...
      if (!acc) {
        violations.push(`Player ${p.publicKey} account missing`);
        continue;
      }
      if (acc.owner !== p.publicKey) {
        violations.push(`Player ${p.publicKey} owner mismatch`);
      }
      if ((acc.networth < 0n)) {
        violations.push(`Player ${p.publicKey} negative networth`);
      }
      playersChecked += 1;
    }
    ...
    for (const team of teams) {
      ...
      for (let i = 0; i < teamAcc.maxMembers; i++) {
        ...
        if (slot && slot.player !== DEFAULT_ADDRESS) {
          populated += 1;
        }
      }
    ...
    if (finalEvent) {
      ...
      for (let i = 0; i < finalEvent.leaderboardCount; i++) {
        ...
        if (!part) {
          violations.push(
            `Event LB rank ${i + 1} (${entry.player}) has no participation acct`,
          );
        }
      }
    }
    ...
    for (const r of createdRallies) {
      ...
      if (!acc) {
        if (r.status === 'cancelled') {
          ...
          violations.push(`Rally ${r.rallyId} (creator=${r.creator.publicKey}) account missing`);
        }
        continue;
      }
      // rally.creator is the owner wallet pubkey, not the player PDA.
      if (acc.creator !== r.creator.publicKey) {
