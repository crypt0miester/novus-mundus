...
export function validateInSpecificTeam(player: PlayerCore, teamPubkey: PublicKey): ValidationResult {
  if (!hasTeam(player)) {
    return invalid('Player is not in a team');
  }
  if (!player.team.equals(teamPubkey)) {
