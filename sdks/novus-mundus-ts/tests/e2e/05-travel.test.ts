...
async function getOriginLocationPda(gameEngine: PublicKey, cityId: number, currentLat: number, currentLong: number) {
  const originGridLat = toGrid(currentLat);
  const originGridLong = toGrid(currentLong);
  return await deriveLocationPda(gameEngine, cityId, originGridLat, originGridLong);
}
