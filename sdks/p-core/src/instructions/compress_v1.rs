...
/// Compress an MPL Core Asset V1.
///
/// ### Accounts:
///   0. `[WRITE]` The asset to compress
///   1. `[OPTIONAL]` The collection
///   2. `[WRITE, SIGNER]` The payer (receives storage fees)
///   3. `[SIGNER, OPTIONAL]` The authority (owner or delegate)
///   4. `[]` The system program
///   5. `[OPTIONAL]` The SPL Noop Program
pub struct CompressV1<'a> {
...
impl CompressV1<'_> {
    ...
    pub fn invoke_signed(&self, signers: &[Signer]) -> ProgramResult {
        let account_metas = [
            ...
            InstructionAccount::readonly(self.collection.address()),
        ...
        write_bytes(&mut instruction_data[0..1], &[17]); // CompressV1 discriminator
