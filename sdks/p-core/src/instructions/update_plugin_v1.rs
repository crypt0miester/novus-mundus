...
/// Serialize plugin update data into buffer. Returns new offset.
pub fn write_plugin_update_data(buf: &mut [MaybeUninit<u8>], mut offset: usize, update: &PluginUpdateData) -> usize {
    match update {
        PluginUpdateData::AttributesSet { attributes } => {
            write_bytes(&mut buf[offset..offset+1], &[6]); // Plugin::Attributes variant
            offset += 1;
        ...
        PluginUpdateData::FreezeDelegateState { frozen } => {
            write_bytes(&mut buf[offset..offset+1], &[1]); // Plugin::FreezeDelegate variant
            offset += 1;
            write_bytes(&mut buf[offset..offset+1], &[*frozen as u8]);
            offset += 1;
        },
        PluginUpdateData::PermanentFreezeDelegateState { frozen } => {
            write_bytes(&mut buf[offset..offset+1], &[5]); // Plugin::PermanentFreezeDelegate variant
            offset += 1;
            write_bytes(&mut buf[offset..offset+1], &[*frozen as u8]);
            offset += 1;
        },
...
impl UpdatePluginV1<'_> {
    ...
    pub fn invoke_signed(&self, signers: &[Signer]) -> ProgramResult {
        ...
        offset = write_plugin_update_data(&mut instruction_data, offset, &self.update);
